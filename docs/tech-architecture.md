...
| 组件 | 技术选型 | 核心作用 | 安全特性 |
|------|----------|----------|----------|
| **跨平台框架** | Electron 28.2.0 | 主进程+渲染进程隔离 | sandbox 启用，contextIsolation 强制，单实例锁 |
| **前端框架** | React 18 + TypeScript 5.4 | 用户界面与交互 | 错误边界隔离，Zod 运行时校验 |
| **状态管理** | Zustand 4.4 | 跨组件状态共享 | 敏感状态内存加密，页面卸载清零 |
| **本地数据库** | better-sqlite3 9.4 | 历史日报与事项存储 | **字段级 AES-256-GCM 加密**，WAL 模式 |
